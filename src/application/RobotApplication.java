...
public class RobotApplication extends RoboticsAPIApplication {
	...
	public void run() {
		...
		MotionBatch dance = new MotionBatch(ptp(Math.PI/2,Math.PI/4,0,-Math.PI/4,Math.PI/2,0,Math.PI/2),ptp(-Math.PI/2,-Math.PI/4,0,Math.PI/4,Math.PI/2,0,Math.PI/2).setBlendingRel(0.5).setJointVelocityRel(0.4));
		while(!END){
