...
public class RobotApplication extends RoboticsAPIApplication {
	...
	public void run() {
		...
		MotionBatch dance = new MotionBatch(ptp(Math.PI/2,Math.PI/2,0,Math.PI/2,Math.PI,0,0).setJointVelocityRel(0.4),ptp(-Math.PI/2,-Math.PI/2,0,-Math.PI/2,-Math.PI/2,0,0)).setJointVelocityRel(0.4).setBlendingRel(0);
		while(!END){
